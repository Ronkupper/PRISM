...
> **New through v2.20.0 — a named operating model.** Recent releases add the layer that runs an engagement end to end: a **PRISM Desk** (one operator-facing surface that stages every step), a symmetric **Setup → rounds → Closure** lifecycle, a five-stage **dispatch round-trip**, engagement deliverables (a plain-language report plus an optional interactive workbook), and a core-slimming refactor that keeps the always-loaded core lean by fetching phase-scoped material on demand. See [How an engagement runs](#how-an-engagement-runs). (Installation is unchanged — see [Quick start](#quick-start).)
...
- **Any other vendor, or one file** — attach `PRISM.md` (or `PRISM_v2_20_0.md` for the version-pinned copy) to a fresh chat.
...
**v2.20.0** — current file: [`PRISM.md`](./PRISM.md). v2.20.0 is a MINOR over v2.19.0 that **re-embeds the Lens Library at v0.16** (Appendix G) and names its new lens in the Independent Validation Dispatch default kit (§4.14). The Lens Library v0.16 adds **LL-D-022 "Does this respect the reader?"** — a reader-respect pass that flags filler which spends the reader's attention and returns nothing (empty preambles, restated prompts, contentless transitions, content-free hedges), completing the document-review family (LL-D-019 / LL-D-020 / LL-D-021) — and curates the `recommended_sources:` on **LL-D-008 "Compared to what?"** and **LL-D-009 "Does it pay back?"**, adding Failory and Start-Up Nation Central — Finder, each shipped with its mandatory bias-and-recency caveats. The only framework behavior change is additive — LL-D-022 joins the §4.14 IVD default lens kit; no construct renumbering, no operating-rule change. The version-pinned snapshot at this tag is [`PRISM_v2_20_0.md`](./PRISM_v2_20_0.md) (byte-identical to PRISM.md at the v2.20.0 tag); previous versions are available via git tags per [`RELEASING.md`](./RELEASING.md).
...
- `PRISM_v{n}.md` — versioned snapshot of PRISM.md at the corresponding tag (e.g., `PRISM_v2_20_0.md`); for git-tag recovery per [`RELEASING.md`](./RELEASING.md). Not the primary install target.
