...
- **Any other vendor, or one file** — attach `PRISM.md` (or `PRISM_v2_19_0.md` for the version-pinned copy) to a fresh chat.
...
**v2.19.0** — current file: [`PRISM.md`](./PRISM.md). v2.19.0 ("Core-slimming refactor") is a MINOR over v2.18.0 — a **projection-only, behavior-preserving** packaging change that slims the always-loaded Skill core by roughly a quarter (~24%). `PRISM.md` stays the canonical monolith; only the projection changes. The phase-, seam-, and condition-scoped mechanics move into four new on-demand bundles keyed to the role that needs them: **`reference/setup.md`** (Setup mechanics — the seven probes, three-layer readiness, the Setup-artifact procedures, strategy stability, onboarding; §6.1–6.6, §7.1, §7.3–7.4, §8.1), **`reference/currency.md`** (point-refresh, the Update session, the session-open currency check; §7.5–7.6, §18.3), **`reference/continuity.md`** (migration handoff, failsafe recovery, defensive migration, missing-handoff recovery; §5.4–5.6, §14 — with a minimal **in-core recovery floor** that stays resident so a session in trouble is never stranded), and **`reference/corpus-access.md`** (the corpus-access dispatch; §4.13). Each gutted area leaves a fail-loud in-core trigger stub with a visible SP-4 self-check load line; the phase → bundle manifest (§3.7.6) and the `SKILL.md` load gate route the loads, and the v2.16 bundle-load integrity layer catches a miss. The execution-hot spine (Standing Principles, Monitors, the triple contract, the dispatch lifecycle, the §6.7 closure gate, the §7.2 lens schema) stays in core. Also folds in one doc-hygiene correction — the Appendix C tag-count summary is reconciled to its list-entry counts (50 / 18 / 81). Additive — no construct renumbering, no operating-rule change. The version-pinned snapshot at this tag is [`PRISM_v2_19_0.md`](./PRISM_v2_19_0.md) (byte-identical to PRISM.md at the v2.19.0 tag); previous versions are available via git tags per [`RELEASING.md`](./RELEASING.md).
