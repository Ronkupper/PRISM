...
PRISM ships on two end-user channels, neither a `git clone`: the **Claude Skill plugin** via the GitHub marketplace (`Ronkupper/PRISM`), and a **single-file attachment**. A versioned filename (`PRISM_v2_0_0.md`, `PRISM_lens_library_v0_9.md`) lets the single file self-document its version wherever it travels — attached to a Claude chat, saved to a phone, shared between collaborators — so you always know what version you're working with from the filename alone, without opening the file or consulting external metadata. The Skill carries its version in `plugins/prism/.claude-plugin/plugin.json` instead (see *Framework release*).
...
4. **Bump the plugin version.** `plugins/prism/.claude-plugin/plugin.json` `version` is the authoritative field Claude Code uses as the update cache key — it must advance on **every** release that should reach installed users; pushing new commits without bumping it does nothing for them. Keep `.claude-plugin/marketplace.json` `plugins[].version` in sync with it (Claude Code resolves `plugin.json` first, so a stale marketplace value can silently mask the real version; a stale marketplace value is also what leaves the desktop **Update** button greyed). If the plugin tree is regenerated by the projection step, confirm both files landed at the new version.
   ```json
...
- [ ] **Lint gate clean** — the CI workflow at `.github/workflows/lint.yml` runs the `scripts/lint/` catalog and reports zero error-severity findings. It runs on **push to `main`, on tags, and on PRs** (not PRs only), so a release tag is itself gated. Coverage as of catalog v5: `PRISM-LINT-01`/`-02` (named-refs-resolve / orphan-anchor) over both the assembled `PRISM.md` **and** the split Skill archive (`lint_cross_file_refs.py`, now CI-wired); `PRISM-LINT-10`–`-13` (package integrity, `lint_package.py`) — frontmatter parses in every shipped `SKILL.md`/command, command field types, `plugin.json`/`marketplace.json` JSON-validity, and `VERSION`==`plugin.json`==marketplace version; plus a package-build step asserting clean ZIP contents (expected root entries, no `__pycache__`/`.pyc`). When Pattern A's schema lands, `PRISM-LINT-03`/`-04`/`-05`/`-07` become active. `PRISM-LINT-06` stays catalog-reserved at info-severity. Info-level `PRISM-LINT-02` (orphan anchors) findings are acceptable and do not block release.
- [ ] **Catalog/framework version coupling** — if this release activates or changes a lint rule, `lint_rules.md` **Catalog version** and `PRISM.md` frontmatter `lint_catalog_version` advance together (the framework bump ships on its own track; the two integer counters may sit at different values off-cycle between the two commits).
