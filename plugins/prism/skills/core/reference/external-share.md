<!-- PRISM v2.21.0 Skill bundle (on-demand reference). External share reference — one-repo-per-engagement, the de-coded share archive, operator-selectable share modes, the image-redaction procedure, and the canonical-source (read-the-repo-not-the-mirror) block (Appendix L). Fetch when preparing an external handoff.
