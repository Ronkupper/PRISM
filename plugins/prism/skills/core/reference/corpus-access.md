<!-- PRISM v2.19.0 Skill bundle (on-demand reference). Corpus-access dispatch (section 4.13) — the investigation-posture Envelope for a targeted lookup against an external reference corpus, brought back caveat-attached. Fetch when the dispatch-builder classifies a pass as corpus-access.
