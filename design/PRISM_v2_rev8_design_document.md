...
Project:            Acme Audit
Master version:     acme_audit_prism2.0_p2.2
...
Attachments:        acme_brief.md, regulatory_context.md
Expected output:    acme_audit_p2.3_gemini.md
...
v2 names this file the **Master** — as in master record, the authoritative canonical version from which derivatives are cut. The filename convention is `[project_name]_prism[version]_master_[phase-derived versioning].md` (e.g., `acme_audit_prism2.0_master_p2.3.md`). The Master is a single file by principle; the plain-chat substrate (§3.1) makes file-based persistence the only available mechanism for cross-session continuity.
